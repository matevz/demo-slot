...
import Slot from "../contracts/Slot.json";
...
//    setState({ swag });
  ...
  const draw = async () => {
    try {
      ...
      setState(prevState => ({...prevState, slot: undefined}));
      try {
        state._slotWrite.on({
          address: state._slotWrite.address,
          topics: [
            state._slot.interface.getEventTopic("Win"),
/*            null,
            ethers.utils.hexZeroPad(state.selectedAddress, 32),
*/          ]
        }, (from, to, tokenId) => {
          console.log("win!");
          setState(prevState => ({...prevState, slot: "won", wonReward: state.maxReward}));
        });

        state._slotWrite.on({
          address: state._slotWrite.address,
          topics: [
            state._slot.interface.getEventTopic("Lose"),
/*            null,
            ethers.utils.hexZeroPad(state.selectedAddress, 32),
*/          ]
        }, (from, to, tokenId) => {
          console.log("lose");
          setState(prevState => ({...prevState, slot: "lose"}));
        });

        const tx = {value: ethers.utils.hexlify(state.ticketPrice), gasPrice: ethers.utils.hexlify(100_000_000_000), gasLimit: ethers.utils.hexlify(100_000)};
        const s = await state._slotWrite.draw(tx);
        const receipt = await s.wait();
      } catch (e) {
        setState(prevState => ({...prevState, slotError: e.message}));
      }
    } catch (e) {
      setState((prevState) => ({
  ...
  /*const _transferTokens = async (to, amount) => {
    ...
    try {
      ...
      const tx = await state._slotWrite.transfer(to, amount);
      setState((prevState) => ({
  ...
  const _initializeEthers = async () => {
    ...
    const _slot = new ethers.Contract(
      contractAddress.Token,
      Slot.abi,
      new ethers.providers.Web3Provider(window.ethereum).getSigner(),
    )
    const _slotWrite = new ethers.Contract(
      contractAddress.Token,
      Slot.abi,
    ...
    _updateRewards(_slot);
  }

  const _updateRewards = async (slot) => {
    const maxReward = (await slot.provider.getBalance(slot.address)).div(256).mul(await slot.maxRewardRatio());
    const ticketPrice = await slot.ticketPrice();
    setState((prevState) => ({
      ...prevState, maxReward, ticketPrice,
    }));
  }
